...
class XRuntimeLogParser:
    ...
    def _parse_knowledge_line(self, line: str) -> KnowledgeError | None:
        ...
        if error_match:
            return KnowledgeError(
                ...
                message=error_match.group("error") or "Unknown retrieval error",
                query_preview="",
                severity="error",
            )
    ...
    def parse_line(self, line: str) -> None:
        ...
        if rbac_violation:
            ...
            if self.verbose:
                print(f"[RBAC VIOLATION] {rbac_violation.session_id} - "
        ...
        if knowledge_error:
            ...
            if self.verbose:
                print(f"[KNOWLEDGE ERROR] Tenant: {knowledge_error.tenant_id} - "
...
def print_text_report(result: LogAnalysisResult) -> None:
    ...
    if result.rbac_violations:
        ...
        print("\n按工具统计 (前10):")
        for tool, count in sorted(tool_stats.items(), key=lambda x: -x[1])[:10]:
    ...
    if result.knowledge_errors:
        ...
        print("\n按租户统计:")
        for tenant, count in sorted(tenant_stats.items(), key=lambda x: -x[1])[:10]:
...
def print_csv_report(result: LogAnalysisResult) -> None:
    ...
    for e in result.knowledge_errors:
        print(f"knowledge,{e.timestamp},{e.tenant_id},{e.error_type},{e.message}")
...
def check_alert_threshold(result: LogAnalysisResult, threshold: int) -> bool:
    ...
    if len(result.rbac_violations) >= threshold:
        print(f"⚠️  ALERT: RBAC 违规次数 ({len(result.rbac_violations)}) 超过阈值 ({threshold})")
        has_alert = True
    
    error_count = sum(1 for e in result.knowledge_errors if e.severity == "error")
