...
def _set_tenant_ctx_for_factory() -> None:
    """Set ``current_tenant`` so the middleware_factory's fail-closed
    defense-in-depth check passes.

    The factory (per Task 1 fix) raises ``TenantIsolationError`` when
    ``auth_enabled`` is True and ``current_tenant`` is unset. These
    tests call the factory directly (bypassing the gateway handler
    that normally sets the contextvar), so we seed a test tenant
    here and clear it after to prevent cross-test leakage.
    """
