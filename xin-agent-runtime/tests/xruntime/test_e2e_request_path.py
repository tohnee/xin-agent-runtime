...
async def app_client(monkeypatch, tmp_path):
    ...
    # Disable auth so the gateway handler uses the backwards-compat
    # path (trusts client-supplied x-tenant-id). These tests exercise
    # the streaming pipeline, not auth enforcement — auth fail-closed
    # is covered separately in test_extension_tenant_failclosed.py.
    e2e_config = XRuntimeConfig()
    e2e_config.server.auth_enabled = False
    ext = create_xruntime_extension(config=e2e_config)
    app = create_app(
        storage=storage,
        message_bus=bus,
