...
class ToolResultCompressionTest(IsolatedAsyncioTestCase):
    ...
    async def test_loop_includes_index_zero(self) -> None:
        """The boundary-finding loop must test i=0 (empty prefix).

        When all blocks exceed the token limit individually, the loop
        should still descend to i=0 to explicitly find the boundary at
        index 0. With the buggy ``range(..., 0, -1)`` stop value,
        i=0 is never tested — the loop stops at i=1.

        We verify by counting ``count_tokens`` calls: with DataBlocks
        (no truncation path), the fixed loop calls count_tokens 3
        times (initial + i=1 + i=0), while the buggy loop calls it
        only 2 times (initial + i=1).
        """
        block1 = DataBlock(
            source=Base64Source(
                data="datablock1",
        ...
        # With the fix (range(..., -1, -1)), the loop tests i=0
        # (empty prefix), so count_tokens is called 3 times:
        #   1. initial full-output check (2 blocks → 200)
        #   2. loop i=1 (1 block → 200)
        #   3. loop i=0 (0 blocks → 0, fits → boundary_index=0)
        # Without the fix (range(..., 0, -1)), i=0 is never tested,
        # so count_tokens is called only 2 times.
        self.assertEqual(
            call_count,
            3,
            f"Expected 3 count_tokens calls (including i=0 test), "
            f"got {call_count}. The loop must descend to i=0.",
        )

        # Both blocks should be offloaded (none reserved, boundary
        # block inserted into offload as inseparable DataBlock).
        self.assertIsNotNone(offload)
        self.assertEqual(len(reserved.output), 0)
        self.assertEqual(len(offload.output), 2)
