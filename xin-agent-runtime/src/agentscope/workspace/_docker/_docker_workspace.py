...
# Values chosen to mirror Vercel Eve's sandbox boundary: drop all
# capabilities, run as non-root, bound RAM/CPU/PIDs, read-only rootfs
# with tmpfs for writable dirs, and isolate from the default bridge.
...
# Env var name substrings that must never leak into the sandbox.
# Mirrors Vercel Eve's credential-brokering boundary: secrets stay on
# the host and are injected per-request by the network firewall, not
# baked into the container env.
...
class DockerWorkspace(WorkspaceBase):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            sandbox_user (`str`, defaults to ``"1000:1000"``):
                UID:GID the container process runs as. Must be
                non-root (UID ≠ 0) to prevent privilege escalation
                via bind-mounted host directories.
            memory_limit (`int`, defaults to 512 MiB):
                Maximum RAM (bytes) the container may use.
            cpu_limit (`int`, defaults to 1 core):
                CPU quota in nanocpus (1e9 = 1 core).
            pids_limit (`int`, defaults to 256):
                Maximum number of processes inside the container.
            network_mode (`str`, defaults to ``"none"``):
                Docker network mode. ``"none"`` disables all
                networking; use a custom network name to allow
                gateway-only egress. Never ``"bridge"`` (default,
                allows container-to-container traffic).
            readonly_rootfs (`bool`, defaults to ``True``):
                Mount the container's root filesystem read-only.
                Writes go to tmpfs (``/tmp``, ``/run``) or the
                bind-mounted ``workdir``.
            sanitize_env (`bool`, defaults to ``True``):
                Filter out environment variables whose names contain
                sensitive substrings (``API_KEY``, ``SECRET``,
                ``TOKEN``, ``PASSWORD``, ``CREDENTIAL``) so secrets
    ...
    async def _create_and_start_container(self) -> None:
        ...
        * **Sandbox hardening** (mirrors Vercel Eve boundary):
          non-root ``User``, ``CapDrop: ["ALL"]``, ``Memory`` /
          ``NanoCpus`` / ``PidsLimit`` quotas, ``SecurityOpt:
          ["no-new-privileges"]``, isolated ``NetworkMode``,
          ``ReadonlyRootfs`` with tmpfs for ``/tmp`` + ``/run``.
        ...
        sanitized_env = self._build_sanitized_env()
        if sanitized_env:
        ...
        if self.readonly_rootfs:
            host_config["Tmpfs"] = {
                "/tmp": f"size={DEFAULT_TMPFS_SIZE}",
                "/run": f"size={DEFAULT_TMPFS_SIZE}",
            }
        if self.host_workdir is not None:
    ...
    def _build_sanitized_env(self) -> list[str]:
        """Build the container ``Env`` list with secrets filtered out.

        When ``sanitize_env`` is ``True`` (default), any key whose
        uppercase form contains one of
        :data:`_SENSITIVE_ENV_PATTERNS` is dropped and a warning is
        logged. This enforces the Vercel-Eve-style credential
        brokering boundary: secrets never enter the sandbox.

        Returns:
            List of ``"KEY=VALUE"`` strings safe to pass to the
            container.
        """
        if not self.env:
            return []
        ...
        for key, value in self.env.items():
            ...
            if any(pat in upper for pat in _SENSITIVE_ENV_PATTERNS):
                logger.warning(
                    "DockerWorkspace: dropping sensitive env var "
                    "%r from container config (credential brokering "
                    "boundary).",
                    key,
                )
                continue
