...
# The forward reference in XRuntimeConfig.approval needs the symbol in
# this module's namespace for pydantic to resolve it.
from ._runtime._middleware._approval import ApprovalConfig  # noqa: E402,F401

# CredentialBrokerConfig is pydantic-only — safe to import at module
# load. The forward reference in XRuntimeConfig.credential_broker needs
# the symbol in this module's namespace.
from ._runtime._credential._config import (  # noqa: E402,F401
    CredentialBrokerConfig,
)

# WorkflowConfig is pydantic-only — safe to import at module load.
# The forward reference in XRuntimeConfig.workflow needs the symbol in
# this module's namespace.
from ._runtime._workflow._config import WorkflowConfig  # noqa: E402,F401
...
class TenantConfig(BaseModel):
    ...
    Args:
        ...
        tool_allowlist (`list[str] | None`):
            Tenant-level tool allowlist.  When set, only tools in this
            list are permitted — request-level ``allowed_tools`` is
            intersected with this list.  ``None`` means no tenant-level
            restriction (all tools permitted by the request are allowed).
        model_allowlist (`list[str] | None`):
            Tenant-level model config allowlist.  When set, only model
            configs named in this list can be used by this tenant —
            model resolution returns ``None`` for configs not in the
            list.  ``None`` means no tenant-level restriction.
    """
...
class XRuntimeConfig(BaseModel):
    ...
    approval: "ApprovalConfig" = Field(
        default_factory=lambda: ApprovalConfig(),
    )
    """HITL approval middleware configuration. When ``enabled``, an
    :class:`ApprovalMiddleware` is injected into the agent middleware
    chain to gate tool calls behind human approval."""
    credential_broker: "CredentialBrokerConfig" = Field(
        default_factory=lambda: CredentialBrokerConfig(),
    )
    """Credential broker configuration. When ``enabled``, the runtime
    issues short-lived, scope/audience-bound credentials per request
    instead of passing long-lived provider secrets through. Secrets
    stay on the host; only the brokered ``credential_id`` crosses the
    sandbox boundary (Vercel-Eve-style credential brokering)."""
    workflow: "WorkflowConfig" = Field(
        default_factory=lambda: WorkflowConfig(),
    )
    """Workflow + checkpoint configuration. When ``enabled``, the
    :class:`CheckpointedOrchestrator` persists a checkpoint after each
    topological layer so a crash mid-workflow loses at most the
    in-flight layer.  Resume loads the latest checkpoint and
    continues with the next pending layer."""
