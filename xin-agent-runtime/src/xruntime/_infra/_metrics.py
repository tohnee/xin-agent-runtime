...
def _escape_label_value(value: str) -> str:
    ...
    The exposition format requires the following characters inside a
    label value to be escaped:

    - backslash (``\\``) -> ``\\\\``
    - double quote (``"``) -> ``\\"``
    - newline (``\\n``) -> ``\\\\n``

    The backslash must be escaped first so that an already-escaped
    sequence is not double-escaped.
    ...
    Returns:
        `str`: The escaped label value, safe to embed inside double
        quotes in a Prometheus exposition line.
    """
    return (
        str(value)
        .replace("\\", "\\\\")
...
class MetricsCollector:
    ...
    def export_prometheus(self) -> str:
        ...
        for tid, count in self._active_sessions.items():
            lines.append(
                f'xruntime_active_sessions{{tenant="'
                f'{_escape_label_value(tid)}"}} {count}',
            )
        ...
        for tool, durations in self._tool_calls.items():
            lines.append(
                f'xruntime_tool_calls_total{{tool="'
                f'{_escape_label_value(tool)}"}} {len(durations)}',
            )
        ...
        for tid, totals in self._tokens.items():
            tid_esc = _escape_label_value(tid)
            lines.append(
                f'xruntime_tokens_total{{tenant="{tid_esc}",'
                f'type="input"}} {totals["input"]}',
            )
            lines.append(
                f'xruntime_tokens_total{{tenant="{tid_esc}",'
                f'type="output"}} {totals["output"]}',
            )
        ...
        for spec, stats in self._subagent_calls.items():
            spec_esc = _escape_label_value(spec)
            lines.append(
                f"xruntime_subagent_calls_total"
                f'{{spec="{spec_esc}",status="success"}} '
                f'{stats["successes"]}',
            )
            lines.append(
                f"xruntime_subagent_calls_total"
                f'{{spec="{spec_esc}",status="failure"}} '
                f'{stats["failures"]}',
            )
        ...
        for spec, stats in self._subagent_calls.items():
            ...
            lines.append(
                ...
                f'{{spec="{_escape_label_value(spec)}"}} {avg:.4f}',
            )
        ...
        for spec, stats in self._subagent_calls.items():
            lines.append(
                ...
                f'{{spec="{_escape_label_value(spec)}"}} '
        ...
        for mw_name, durations in self._middleware_latency.items():
            ...
            lines.append(
                ...
                f'{{middleware="'
                f'{_escape_label_value(mw_name)}"}} {avg:.4f}',
            )
