...
- :func:`mount_protocol_adapters` — mounts the four protocol adapter
  routes (``/v1/messages``, ``/v1/claude-code/query``,
  ``/v1/opencode``, ``/v1/chat/completions``) onto an existing AS
  FastAPI app.
...
from ._openai_adapter import OpenAIChatAdapter
...
_ROUTE_PROTOCOL_MAP: dict[str, ProtocolType] = {
    ...
    "/v1/chat/completions": ProtocolType.OPENAI,
}
...
# Bounded stream queue to prevent unbounded memory growth when the
# client (consumer) is slower than the event producer.  When full,
# the feeder task blocks on ``put``, providing natural backpressure.
STREAM_QUEUE_MAXSIZE = 1000


def _default_adapters() -> AdapterRegistry:
    """Build a registry with all four protocol adapters.
...
def create_xruntime_extension(
    ...
    async def middleware_factory(
        ...
        # Fail-closed tenant resolution (defense-in-depth).
        #
        # The gateway handler is the authoritative fail-closed point:
        # it rejects with 401 when auth is enabled but no principal
        # was resolved, and with 403 on tenant-id spoofing. By the time
        # the middleware_factory runs (inside ``ChatService.run`` via
        # the gateway handler's ``_stream``), ``current_tenant`` MUST
        # be set.
        #
        # This check is defense-in-depth: if a bug in the gateway
        # handler lets ``current_tenant`` be unset while
        # ``auth_enabled`` is True, we refuse to proceed rather than
        # silently fall back to ``"default"`` — which would let the
        # request read/write the ``default`` tenant's namespace on
        # shared Redis infrastructure (a cross-tenant leak vector).
        #
        # When auth is disabled (local-dev / single-tenant embed),
        # we preserve the existing fallback to ``tenant_id`` /
        # ``"default"`` for backwards compatibility.
        ctx_tenant = current_tenant.get()
        if ctx_tenant is None and config.server.auth_enabled:
            from .._infra._tenant import TenantIsolationError

            raise TenantIsolationError(
                "Auth is enabled but current_tenant is unset inside "
                "middleware_factory — the gateway handler should have "
                "set it or rejected the request. Refusing to fall "
                "back to 'default' to prevent cross-tenant leak.",
            )
        effective_tenant = ctx_tenant or tenant_id or "default"
        ...
        # --- HITL approval gating (on_acting) ---
        # When ``config.approval.enabled`` is True, an ApprovalMiddleware
        # is appended so tool calls can be gated behind a human approver.
        # The approval callback is resolved from the config's
        # ``approval_callback`` attribute if set, otherwise the middleware
        # is attached with no callback (any strategy requiring approval
        # will time out — useful for dry-run / observability).
        if (
            getattr(config, "approval", None) is not None
            and config.approval.enabled
        ):
            from .._runtime._middleware._approval import (
                ApprovalMiddleware,
            )

            approval_state = await state_cache.get_approval_state_cache()
            approval_callback = getattr(
                config.approval,
                "approval_callback",
                None,
            )
            # Predicate is only meaningful for PREDICATE strategy.
            predicate = getattr(config.approval, "predicate", None)
            middlewares.append(
                ApprovalMiddleware(
                    strategy=config.approval.strategy,
                    approval_callback=approval_callback,
                    timeout_seconds=config.approval.timeout_seconds,
                    tenant_id=effective_tenant,
                    user_id=user_id,
                    predicate=predicate,
                    always_require_tools=set(
                        config.approval.always_require_tools,
                    ),
                    never_require_tools=set(
                        config.approval.never_require_tools,
                    ),
                    state_cache=approval_state,
                ),
            )
...
class _GatewayState:
    ...
    it. Credential and agent ids are cached per (tenant, user, key) so
    repeated requests reuse persisted records instead of creating
    duplicates. The ``tenant_id`` is part of the cache key so that two
    tenants with the same ``user_id`` (e.g. both have an "alice") do
    not alias each other's records — the storage layer is tenant-
    prefixed via ``key_config``, so the in-memory cache must be too.
    """
    ...
    def __init__(
        ...
        # (tenant_id, user_id, provider_name, api_key) -> credential_id
        self._credential_cache: dict[tuple[str, str, str, str], str] = {}
        # (tenant_id, user_id, agent_name) -> (agent_id, system_prompt,
        # max_iters, model_config_name)
        self._agent_cache: dict[
            tuple[str, str, str], tuple[str, str, int, str | None]
        ] = {}
    ...
    def credential_id(
        ...
        return self._credential_cache.get(
            (tenant_id, user_id, provider_name, api_key),
        )
    ...
    def agent_cache(
        ...
        """Return cached (agent_id, system_prompt, max_iters,
        model_config_name), if any.
    ...
    def cache_agent(
        ...
        Args:
            ...
            model_config_name (`str | None`): The model config name.
        """
        self._agent_cache[(tenant_id, user_id, agent_name)] = (
            agent_id,
            system_prompt,
            max_iters,
            model_config_name,
    ...
    *,
    ...
    def _make_handler(
        ...
        async def _handler(request: Request) -> Any:
            ...
            gw_log = logging.getLogger("xruntime.gateway")
            ...
            gw_log.info(
                "Request received: protocol=%s, tenant=%s, user=%s, "
                "session=%s, tool_mode=%s",
                protocol_type.value,
                xrt_request.tenant_id,
                xrt_request.user_id,
                xrt_request.session_id,
                xrt_request.tool_mode.value,
            )
            ...
            # Fail-closed + anti-spoofing tenant resolution.
            #
            # When auth is enabled (``config.server.auth_enabled`` is
            # True) the gateway handler expects AuthMiddleware to have
            # resolved a principal and attached it to
            # ``request.state.principal``. If ``principal`` is None
            # here it means the auth middleware was not mounted (a
            # misconfiguration) or a middleware ordering bug let the
            # request through — we must reject with 401 rather than
            # fall back to ``"default"`` (which would leak across
            # tenants on shared infrastructure).
            #
            # When a principal is present and the client supplied an
            # ``x-tenant-id`` header that does not match the
            # principal's tenant, reject with 403 — the client is
            # attempting to spoof another tenant's namespace.
            #
            # When auth is disabled (local-dev / single-tenant embed)
            # we trust the client-supplied ``xrt_request.tenant_id``
            # for backwards compatibility, but log a warning so the
            # operator knows tenant isolation is not enforced.
            ...
            if auth_enabled:
                if principal is None:
                    ...
                    gw_log.warning(
                        "Auth is enabled but no principal was set on "
                        "request.state; rejecting to prevent "
                        "cross-tenant leak (check that AuthMiddleware "
                        "is mounted on path=%s).",
                        request.url.path,
                    )
                    raise HTTPException(
                        status_code=401,
                        detail="Authentication required but no "
                        "principal was resolved "
                        "(auth middleware misconfiguration).",
                    )

                # Anti-spoofing: a mismatch between the client-supplied
                # ``x-tenant-id`` and the authenticated principal's
                # tenant means the client is attempting to act as
                # another tenant. Reject with 403 rather than silently
                # overriding or trusting the spoofed value.
                if xrt_request.tenant_id != principal.tenant_id:
                    from fastapi import HTTPException

                    gw_log.warning(
                        "Tenant spoofing rejected: principal "
                        "tenant=%s, client-supplied x-tenant-id=%s "
                        "(path=%s, user=%s).",
                        principal.tenant_id,
                        xrt_request.tenant_id,
                        request.url.path,
                    ...
                    raise HTTPException(
                        status_code=403,
                        detail="Tenant id in request does not match "
                ...
                gw_log.info(
                    "Authenticated principal: tenant=%s, " "user=%s, role=%s",
                    effective_tenant,
                    effective_user,
                    getattr(principal, "role", "unknown"),
                )
            else:
                # Auth disabled: trust the client-supplied tenant id
                # for backwards compatibility (single-tenant / local
                # dev embeds). Log a warning so the operator knows
                # tenant isolation is not enforced.
                effective_tenant = xrt_request.tenant_id
                effective_user = xrt_request.user_id
                gw_log.warning(
                    "Auth is disabled; trusting client-supplied "
                    "x-tenant-id=%s (path=%s). Enable "
                    "server.auth_enabled in production to enforce "
                    "tenant isolation.",
                    effective_tenant,
                    request.url.path,
                )
            ...
            gw_log.debug(
                "current_tenant contextvar set to %s",
                effective_tenant,
            )
            ...
            try:
                ...
                # the principal and tenant config.
                authorized_kb_ids: list[str] = []
                if principal is not None and hasattr(principal, "kb_ids"):
                    authorized_kb_ids = list(principal.kb_ids)

                # Look up the tenant's tool allowlist from config.
                # Use a dict cache to avoid O(N) linear scan on
                # every request.
                if not hasattr(state, "_tenant_cfg_index"):
                    state._tenant_cfg_index = {
                        tc.id: tc for tc in state.config.tenants
                    }
                tenant_tool_allowlist: set[str] | None = None
                tenant_cfg = state._tenant_cfg_index.get(
                    effective_tenant,
                )
...
async def _serialize_stream(
    ...
    async def _feeder() -> None:
        ...
        try:
            ...
            gw_log.error("feeder failed for session %s: %s", session_id, exc)
        finally:
...
def _observe_task(task: asyncio.Task) -> None:
    ...
    if exc is not None:
        try:
            ...
            print(
                f"[xruntime] unlogged task exc: {exc!r}",
                file=sys.stderr,
            )
...
async def _materialize_session(
    ...
    404. Credentials and agents are cached per (tenant, user, key) for
    reuse; sessions are created fresh unless the request resumes an
    existing one (``session_id`` or Claude Code
    ...
    _mat_log = logging.getLogger("xruntime.gateway.materialize")

    model_config_name = _resolve_model_config_name(state, request)
    _mat_log.info(
        "Resolving model: tenant=%s, model_config=%s",
        tenant_id,
        model_config_name,
    )
    ...
    if provider is None:
        _mat_log.warning(
            "Model resolution FAILED for tenant=%s, model_config=%s "
    ...
    _mat_log.info(
        "Model resolved: tenant=%s, provider=%s, model=%s",
        tenant_id,
        provider.name,
        provider.model,
    )

    # 1. Credential (cached per tenant + user + provider + api_key)
    ...
    # 2. Agent (cached per tenant + user + name; re-upsert on change)
...
async def _ensure_agent(
    ...
    The in-memory cache is keyed by ``(tenant_id, user_id,
    agent_name)`` so two tenants sharing a ``user_id`` cannot alias
    each other's cached ``agent_id`` — the storage layer is tenant-
    prefixed via ``key_config`` resolved from ``current_tenant``, and
    the cache must match that scoping.

    Args:
        state (`_GatewayState`): The gateway state.
        storage (`StorageBase`): The AS storage backend.
        tenant_id (`str`): The tenant id (scopes the cache key).
        user_id (`str`): The user id.
        agent_name (`str`): The agent name.
        system_prompt (`str`): The system prompt.
        max_iters (`int`): The max iterations.
    ...
    cached = state.agent_cache(tenant_id, user_id, agent_name)
    if (
        cached is not None
        and cached[1] == system_prompt
        and cached[2] == max_iters
        and cached[3] == model_config_name
    ):
        return cached[0]
