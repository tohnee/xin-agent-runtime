...
class MiddlewareStateCache:
    ...
    def __init__(
        ...
        # NOTE: _knowledge_mw cache intentionally removed — see
        # get_knowledge_middleware(). Per-call instances are created
        # fresh to avoid cross-tenant scope leakage.
        self._knowledge_registry: Any | None = None
        self._metrics: Any | None = None
        self._langfuse_exporter: Any | None = None
        self._approval_state_cache: Any | None = None
    ...
    async def get_knowledge_middleware(
        ...
        """Return a per-call knowledge middleware, or None.

        The knowledge *registry* is initialized once and cached (it
        is expensive to build and is tenant-shared).  However, a
        fresh :class:`KnowledgeMiddleware` instance is created for
        every call so that per-call fields (``user_id``, ``kb_ids``,
        ``role``) cannot leak across concurrent or sequential
        requests — mutating a shared singleton previously caused
        ...
        async with self._lock:
            ...
            if self._knowledge_registry is None:
                from .._runtime._knowledge._adapter import (
                    get_default_factory,
                )
            ...
            # Create a fresh per-call instance — never mutate a shared
            # singleton, which previously caused cross-tenant leakage
            # of user_id/kb_ids/role under concurrent requests.
    ...
    async def get_approval_state_cache(self) -> Any:
        """Return the shared approval state cache, creating it on first use.

        Used by the ``once`` strategy of :class:`ApprovalMiddleware` so
        that a tool approved in turn N does not block again in turn N+1
        of the same session.  The cache is a singleton scoped to this
        :class:`MiddlewareStateCache` (one per tenant).

        Returns:
            `ApprovalStateCache`: The shared approval state cache.
        """
        if self._approval_state_cache is not None:
            return self._approval_state_cache
        async with self._lock:
            if self._approval_state_cache is not None:
                return self._approval_state_cache
            ...
            self._approval_state_cache = ApprovalStateCache()
        return self._approval_state_cache
