...
_logger = logging.getLogger("xruntime.server")
...
def build_xruntime_app(config: XRuntimeConfig | None = None) -> Any:
    ...
    # Wrap storage and message bus with tenant-aware wrappers so that
    # each request's Redis keys / channels are automatically prefixed
    # with ``tenant:{tid}:`` based on the active request's tenant
    # context (via ``current_tenant`` contextvar). This provides full
    # multi-tenant isolation on shared Redis infrastructure.
