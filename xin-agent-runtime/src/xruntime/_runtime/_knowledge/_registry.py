...
def _chunk_in_scope(chunk: KnowledgeChunk, query: KnowledgeQuery) -> bool:
    ...
    Both ``tenant_id`` and ``kb_id`` are checked fail-closed: chunks
    missing either field are rejected to prevent cross-tenant /
    cross-KB data leakage.
    ...
    # Fail-closed: chunks without tenant_id metadata are rejected
    ...
    if tenant_id != query.tenant_id:
        return False
    # Fail-closed for kb_id: when the query scopes by kb_ids, chunks
    # must carry a matching kb_id. Chunks missing kb_id (or carrying
    # None / empty string) are rejected rather than defaulted to
    # "default", so unscoped indexes can't leak into authorized KBs.
    if query.kb_ids:
