...
def _get_tenant_model_allowlist(
    config: "XRuntimeConfig",
    tenant_id: str,
) -> set[str] | None:
    """Look up a tenant's model allowlist from config.
    ...
    Returns:
        `set[str] | None`:
            The allowlist as a set, or ``None`` if the tenant has no
            allowlist (no restriction) or the tenant is not found.
    """
    for tenant in config.tenants:
        if tenant.id == tenant_id:
            if tenant.model_allowlist is not None:
                return set(tenant.model_allowlist)
            return None
    return None
...
class ModelResolver:
    ...
    def resolve(
        ...
        When ``tenant_id`` is provided, the tenant's ``model_allowlist``
        (if set) is enforced — configs not in the allowlist return
        ``None`` even if globally configured.
        ...
        Args:
            ...
            tenant_id (`str | None`):
                Tenant id for allowlist enforcement.  ``None``
                skips tenant-level checks.
    ...
    def resolve_provider(
        ...
        Args:
            ...
            config (`XRuntimeConfig | None`):
                The XRuntime config for fallback resolution.
            tenant_id (`str | None`):
                Tenant id for allowlist enforcement.
        ...
        return self._resolve_source(
            model_config_name, config, tenant_id=tenant_id
        )
    ...
    def _resolve_source(
        ...
        *,
        ...
        When ``tenant_id`` is provided and the tenant has a
        ``model_allowlist``, only allowlisted config names are
        permitted — non-allowlisted names return ``None`` even if
        globally configured.
        ...
        """
        # --- Tenant allowlist check (early-out for denied configs) ---
        if tenant_id is not None and config is not None:
            allowlist = _get_tenant_model_allowlist(config, tenant_id)
            if allowlist is not None and model_config_name is not None:
                if model_config_name not in allowlist:
                    return None
